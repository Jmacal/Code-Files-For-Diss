...
class InterfuserController(object):
    ...
    def get_predicted_vehicle_location(self, x, y, steering_angle, yaw, v, timestamp):
    	wheel_heading = yaw + steering_angle
    	wheel_traveled_dis = v * (timestamp - self.previous_timestamp)
    ...
    def run_step(
        ...
        if speed > 0.2:
        	steering_list = self.steering_previous + self._steering_list * self._pi/180 # List of steering angles in the neighbourhood (left and right) of previous steering angle
        	...
        	for i in range(len(steering_list)):
        		predicted_vehicle_location = self.get_predicted_vehicle_location(x, y, steering_list[i], yaw, v, timestamp) # Get predicted vehicle location based on its current state and control input (i-th steering angle from the list)
        ...
        self.steering_previous = steer # Update previous steering angle value
